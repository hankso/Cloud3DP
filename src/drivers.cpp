...
uint32_t led_color(uint8_t idx) {
    led_code_t *led = led_get_code(idx); if (!led) return 0xdeadbeef;
...
void i2c_detect() {
    ...
    for (uint8_t addr = 0; addr < 0x80; addr++) {
        if (addr % 0x10 == 0) printf("%02X", addr);
...
void spi_initialize() {
    spi_bus_config_t hspi_busconf = {
        ...
        .quadwp_io_num = PIN_HQDWP,
        .quadhd_io_num = PIN_HQDHD,
        .max_transfer_sz = sizeof(spi_pin_data),
    ...
    ESP_ERROR_CHECK( spi_bus_initialize(HSPI_HOST, &hspi_busconf, 1) );
